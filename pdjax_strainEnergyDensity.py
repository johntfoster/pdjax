...
def init_problem(bar_length: float = 20.0,
	...
	left_bc_region = left_bc_region[left_bc_region != 0]
	right_bc_region = jnp.asarray(tree.query_ball_point(pd_nodes[-1, None], r=(2.0 * horizon ), p=2, eps=0.0)).sort()
	right_bc_region = right_bc_region[right_bc_region != (num_nodes - 1)]
...
if __name__ == "__main__":
    ...
    results = _solve(params, state, opt_thick_strain_energy_density, forces_array=forces_array, allow_damage=allow_damage, max_time=float(max_time))
...
def loss(params, state, thickness_vector:Union[float, jax.Array], forces_array:Union[float, jax.Array], allow_damage:bool, max_time:float):
	...
	w_sum = 0.9 # Weight for mean damage (e.g., 70% focus on overall average)
	w_max = 0.10  # Weight for max damage (e.g., 30% focus on peaks)

	#loss_value = damage.sum()  #
	#loss_value = jnp.linalg.norm(damage, ord=1)
	loss_value = jnp.linalg.norm(damage, ord=2)
	#eps = 1e-2
	#loss_value = jnp.sqrt(jnp.sum(damage * damage) + eps)

	#max_damage = jnp.max(damage)    # Maximum damage value across all nodes and saved time steps

	#loss_value = sum_damage * w_sum + thickness_vector.sum() * w_max
...
if __name__ == "__main__":
    ...
    # Solve the problem with initial thickness
    thickness0 = ensure_thickness_vector(thickness0, num_elems)
    results = _solve(params, state, full_thickness, forces_array=forces_array, allow_damage=allow_damage, max_time=float(max_time))
    jax.debug.print("allow_damage in main: {a}", a=allow_damage)
